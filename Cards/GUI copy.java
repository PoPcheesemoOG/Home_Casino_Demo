...
public class GUI extends Application {
	...
	private void playPoker(List<GameCard> playerChoice, List<GameCard> deck, double betSize, Text bankTf, Text display, Group cards, List randoms, VBox vBox1) {
//		int selectedCard;
//		for (int i = 0; i < playerChoice.size(); i++) {
//			selectedCard = playerChoice.get(i).getNumber();
//			cards.getChildren().remove(playerChoice.get(i));
//			cards.getChildren().retainAll(playerChoice);
//		}
		cards.getChildren().retainAll(playerChoice);
		randoms.addAll(cards.getChildren());
		for (int i = cards.getChildren().size(); i < 5; i++) {
	...
	private void newPoker() throws Exception {
		...
		btPlayPoker.setOnAction(e -> {
			...
			playPoker(playerChoice, deck, betSize, bankTf, display, cards, randoms, vBox1);
			pane.setCenterPane(vBox1);
